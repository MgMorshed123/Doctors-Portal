    ...
    <div className="container mx-auto px-4">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {staticData.upcomingEvents.map((event, index) => (
          <motion.div
            key={event.id}
            className="bg-background-light p-6 rounded-lg hover:shadow-lg transition-shadow dark:bg-primary-dark/10"
          ...
          >
            ...
            <h3 className="font-bold dark:text-text-dark mb-2">
              {event.title}
            </h3>
            <p className="text-primary-light font-semibold mb-2">
              {new Date(event.date).toLocaleDateString()}
            </p>
            <p className="dark:text-text-dark text-sm">{event.description}</p>
          </motion.div>
        ))}
      </div>
    </div>
